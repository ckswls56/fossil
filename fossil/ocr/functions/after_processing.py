...
def after_processing(before_result: list):
    ...
    #LAST_DOSE_CHAR = os.getenv('USERNAME').split(',')
    ...
    # result(String List)에서 한글, 숫자, 영어 외의 모든 문자 제거
    # re.IGNORECASE를 사용하여 대소문자 구분 없앰
    middle_result = [re.sub(r"[^가-힣0-9a-z]", "", x, flags=re.IGNORECASE) for x in middle_result]
